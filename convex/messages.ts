...
export const sendTextMessage = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.content.startsWith('@ai')) {
            await ctx.scheduler.runAfter(0, api.gemini.chats, {
        ...
        const messagesWithSender = await Promise.all(
            messages.map(async (message) => {
                if (message.sender === 'Gemini') {
                    ...
                    return { ...message, sender: { name: 'Gemini', image } };
                }
                let sender;
...
// sk-49e8iZSoVXhV7sdlIZKCLxHCLCxyl2broRQ9dO3uETtaqg2K