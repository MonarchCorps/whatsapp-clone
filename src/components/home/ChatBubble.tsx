...
const applyCustomStyles = (text: string) => {
    const listText = text.replace(/^\s*\*\s+/gm, '• ');
    const boldRegex = /\*\*(.*?)\*\*/g;

    const processedText = listText.split(boldRegex).map((part: any, index: number) => {
        if (index % 2 !== 0) {
            return <span key={index} style={{ fontWeight: 'bold', color: '#fff' }}>{part}</span>;
        }
        return part;
    });
